...
import argparse
# import p
# from p.common.logger import configure
from stable_baselines3.common.utils import set_random_seed
from stable_baselines3 import A2C, DDPG, PPO, SAC, TD3
from finrl.config_tickers import DOW_30_TICKER
# from finrl.config_tickers import NAS_100_TICKER as DOW_30_TICKER

from finrl.meta.preprocessor.yahoodownloader import YahooDownloader
from finrl.meta.preprocessor.preprocessors import FeatureEngineer, data_split
from env.marginEnv import MarginTradingEnv
from frac_featurs import *
...
# d_values = [0.10 , 0.20 , 0.30 , 0.40, 0.50 , 0.60, 0.70 , 0.80 , 0.9 ]  # Specify the 'd' values for differentiation
d_values = [i / 100 for i in range(1, 100)]
tau = 0.0005  # Cutoff threshold value
...
if os.path.exists('./datasets/data.csv'):
    processed = pd.read_csv('./datasets/data.csv', index_col=0).sort_values(by=['date', 'tic'])
    processed.set_index('date')
else:
    ...
    for ticker, group in df.groupby('tic'):
        stock_dfs[ticker] = group.reset_index(drop=True)

    # Group by 'tic' and create separate DataFrames for each stock
    stock_dfs = {}
    for ticker, group in df.groupby('tic'):
        # Calculate returns
        group['Returns'] = (group['close'] - group['close'].shift(1))

        # Calculate log returns
        # group['Log_Returns'] = np.log(group['close']) - np.log(group['close'].shift(1))

        # Apply transformations
        for d in d_values:
            # Compute differentiated returns for each 'd' value
            col_name = f'd_{d:.2f}_returns'
            group[col_name] = ts_differencing_tau(group['close'], d, tau)

        # Store the modified DataFrame in the dictionary
        stock_dfs[ticker] = group
    # Concatenate all DataFrames in the stock_dfs dictionary into one DataFrame
    df = pd.concat(stock_dfs.values(), ignore_index=True)
...
# d_values = [0.6, 0.7,0.8,0.9,0.91,0.92,0.93,0.94,0.95,0.96,0.97,0.98,0.99]  # Specify the 'd' values for differentiation
...
state_space = 2 * 3 + 2 * stock_dimension + len(
    INDICATORS) * stock_dimension  # cash, long, short, 30 close, 30 holding shares, 30 tech
...
# model_a2c = agent.get_model("ppo", model_kwargs = model_parameters, seed = seed)
# model_a2c = agent.get_model("ppo", model_kwargs = model_parameters, seed = seed)
# model_a2c = agent.get_model("ppo", model_kwargs = model_parameters, seed = seed)
# model_a2c = agent.get_model("ppo", model_kwargs = model_parameters, seed = seed)
